...
def missing_values(df):
  ...
  # cleaning missing numeric values with the mean
  # if there are more than 40% of the values missing the column is unuseable and get dropped
  Rows = len(df)
  nan_threshold = 0.4
  for col in num_cols:
  ...
  # cleaning missing categorical values with the most used value in the column
  # if there are more than 40% of the values missing the column get dropped
  for col in cat_cols:
    NaNs = df[col].isnull().sum()
    print(f"Before cleaning there are {NaNs} missing values in {col}.")
